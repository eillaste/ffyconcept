...
namespace DAL.Base.EF.Repositories
{
        ...
        where TDbContext: DbContext
    {
        ...
        protected IQueryable<TDomainEntity> CreateSpecialQuery(TKey? userId = default, bool noTracking = true)
        {
            var query = RepoDbSet.AsQueryable();
            // TODO: validate the input entity also
            if (userId != null && !userId.Equals(default) && 
        ...
        /*
        public virtual async Task<IEnumerable<TEntity>> GetAllAsyncNoGuid()
        {
        ...
        public virtual TDalEntity Remove(TDalEntity entity, TKey? userId = default)
        {
            ...
            if (userId != null 
                && !userId.Equals(default) //WTF no idea why this works but it works (probably explained in future)
                && typeof(IDomainAppUserId<TKey>).IsAssignableFrom(typeof(TDomainEntity)) //WTF no idea why this works but it works (probably explained in future)
                &&  !((IDomainAppUserId<TKey>) entity).AppUserId.Equals(userId))
            {
                Console.WriteLine("sitah2da");
            ...
            var a = Mapper.Map(RepoDbSet.Remove(Mapper.Map(entity)!).Entity)!;
